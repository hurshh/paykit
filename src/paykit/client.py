"""PayKitClient - Main SDK entry point."""
...
from paykit.core.circle_client import CircleClient
from paykit.core.config import Config
from paykit.core.exceptions import PaymentError, ValidationError

if TYPE_CHECKING:
    from paykit.providers.base import WalletProvider
...
from paykit.guards.base import PaymentContext
from paykit.guards.manager import GuardManager
from paykit.intents.service import PaymentIntentService
from paykit.ledger import Ledger, LedgerEntry, LedgerEntryStatus
from paykit.payment.batch import BatchProcessor
from paykit.payment.router import PaymentRouter
from paykit.protocols.gateway import GatewayAdapter
from paykit.protocols.transfer import TransferAdapter
from paykit.protocols.x402 import X402Adapter
from paykit.storage import get_storage
from paykit.wallet.service import WalletService
from paykit.webhooks import WebhookParser


class PayKit:
    """
    Main client for PayKit SDK.

    Multi-tenant, vendor-agnostic design for AI agent payments.
    
    Supports multiple wallet providers:
    - Circle (default): Developer-Controlled Wallets
    - Coinbase: Coming soon
    
    Example:
        >>> from paykit import PayKit
        >>> client = PayKit()  # Uses Circle by default
        >>> result = await client.pay(wallet_id="...", recipient="0x...", amount=10)
    ...
    def __init__(
        ...
        """
        Initialize PayKit client.

        Args:
            circle_api_key: Circle API key (or from CIRCLE_API_KEY env). Ignored if provider is set.
            entity_secret: Entity secret for signing (or from ENTITY_SECRET env). Ignored if provider is set.
            network: Target blockchain network
            log_level: Logging level (default INFO). Set to logging.DEBUG for full traceability.
            provider: Optional wallet provider instance. If not provided, uses Circle.
        """
        # Determine log level
        if log_level is None:
            log_level = os.environ.get("PAYKIT_LOG_LEVEL", "INFO")

        # Configure logging immediately
        from paykit.core.logging import configure_logging, get_logger

        configure_logging(level=log_level)
        self._logger = get_logger("client")
        self._logger.info(f"Initializing PayKit SDK (Network: {network.value})")
        ...
        if circle_api_key and not entity_secret:
            ...
            try:
                from paykit.onboarding import auto_setup_entity_secret
        ...
        # Store the provider for vendor-agnostic access
        self._provider = provider
        if provider is not None:
            self._logger.info(f"Using custom provider: {provider.provider_type.value}")
    ...
    @property
    def provider(self) -> WalletProvider | None:
        """
        Get the wallet provider for vendor-agnostic operations.
        
        Returns None if using the default Circle integration via CircleClient.
        Use this for direct provider access when building provider-agnostic code.
        
        Example:
            >>> if client.provider:
            ...     wallet = await client.provider.create_wallet(set_id, "ETH-SEPOLIA")
        """
        return self._provider

    async def __aenter__(self) -> PayKit:
