"""
PayKit SDK Onboarding Utilities.
...
Usage:
    >>> from paykit.onboarding import quick_setup
...
def get_config_dir() -> Path:
    """
    Get the platform-specific config directory for PayKit.

    Returns:
        Path to config directory:
        - Linux: ~/.config/paykit/
        - macOS: ~/Library/Application Support/paykit/
        - Windows: %APPDATA%/paykit/
...
def create_env_file(
    ...
    env_content = f"""# PayKit Configuration
CIRCLE_API_KEY={api_key}
ENTITY_SECRET={entity_secret}
PAYKIT_NETWORK={network}
"""
...
def quick_setup(
    ...
    - Recovery file in secure config directory (~/.config/paykit/)
...
"""
    ...
    print()
    print("Ready to use:")
    print("  from paykit import PayKit")
    print("  client = PayKit()")
...
def auto_setup_entity_secret(
    ...
    Called automatically by PayKit client when ENTITY_SECRET is missing.
    Saves recovery file to secure config directory (~/.config/paykit/).
    ...
    """
    log = logger or logging.getLogger("paykit.onboarding")
