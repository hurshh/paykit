"""
Configuration management for PayKit SDK.
...
class Config:
    ...
    def from_env(cls, **overrides: Any) -> Config:
        ...
        network_str = overrides.get("network") or _get_env_var(
            "PAYKIT_NETWORK", default="ARC-TESTNET"
        )
        ...
        default_wallet_id = overrides.get("default_wallet_id") or _get_env_var(
            "PAYKIT_DEFAULT_WALLET"
        )

        log_level = overrides.get("log_level") or _get_env_var(
            "PAYKIT_LOG_LEVEL", default="INFO"
        )

        env = overrides.get("env") or _get_env_var("PAYKIT_ENV", default="development")
