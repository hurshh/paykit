"""
Unit tests for PayKit Client (Multi-tenant).
...
from paykit.client import GuardManager, PayKit
...
@pytest.fixture
def client(mock_env) -> PayKit:
    """Create client with mocked environment."""
    return PayKit(network=Network.ARC_TESTNET)
...
class TestGuardManager:
    ...
    async def test_add_guard_for_wallet(self):
        from paykit.storage.memory import InMemoryStorage
...
class TestLedgerProperty:
    ...
    async def test_ledger_records_entries(self, client):
        from paykit.ledger import LedgerEntry
...
class TestPayBlocked:
    ...
    async def test_pay_blocked_records_to_ledger(self, client):
        ...
        from paykit.ledger import LedgerEntryStatus
...
class TestPayRequiresWallet:
    ...
    async def test_pay_empty_wallet_raises(self, client):
        from paykit.core.exceptions import ValidationError
